...
{	
    ...
    if (!error)
	{
		...
		//aceptable input
		if(lower){
            pizzasUpdated = pizzaManager.updatePizzasIfLower(price, increment);
            response.writeHead(200, headers);
            response.end(JSON.stringify(pizzasUpdated));
        }else{
            pizzaManager.updatePizzasIfNotLower(price, increment);
            response.writeHead(200, headers);
            response.end(JSON.stringify(pizzasUpdated));
        }
		//if insertion works correctly
